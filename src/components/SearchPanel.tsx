  ...
  return (
    <section className="relative z-40 rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="space-y-3">
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-700">Add to mix</p>
        <h2 className="text-2xl font-semibold text-slate-950">Search or paste a link</h2>
        <p className="text-sm leading-6 text-slate-600">
          Type a lofi cue, rain loop, jazz texture, or paste a direct YouTube URL. Selecting a result drops it
      ...
      <div className="relative mt-5">
        ...
        >
          ...
          >
            {isResolvingInput ? "Adding channel..." : "Add top result or pasted video"}
          </button>
        </form>
        ...
        {showResults ? (
          <div className="absolute inset-x-0 top-[calc(100%+14px)] z-[120] overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-xl">
            {isSearching ? <p className="px-4 py-5 text-sm text-slate-500">Searching YouTube...</p> : null}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="line-clamp-2 text-sm font-medium text-slate-900">{result.title}</p>
                        <p className="mt-1 text-xs text-slate-500">{result.channelTitle}</p>
                        <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-slate-400">
                          ...
                          {isAlreadyAdded ? <span className="text-blue-700">Already in mix</span> : null}
                        </div>
                      </div>
                    </button>
                  );
            ...
            {!isSearching && searchError ? <p className="px-4 py-5 text-sm text-red-600">{searchError}</p> : null}
            ...
            deferredQuery.length >= 2 ? (
              <p className="px-4 py-5 text-sm text-slate-500">No matching videos yet. Refine the search phrase.</p>
            ) : null}
          </div>
        ) : null}
      </div>
      ...
      {!canAddMore ? (
        <p className="mt-3 text-sm text-slate-500">The grid is full. Remove a channel before adding another one.</p>
      ) : null}
    </section>
  );
}
