...
function collectVideoRenderers(items: unknown[]) {
  ...
  for (const item of items) {
    ...
    const itemSection = itemRecord.itemSectionRenderer;
    if (!itemSection || typeof itemSection !== "object") {
      continue;
    }

    const sectionContents = Array.isArray((itemSection as Record<string, unknown>).contents)
      ? ((itemSection as Record<string, unknown>).contents as unknown[])
      : [];
    ...
    for (const sectionItem of sectionContents) {
      ...
      const sectionRecord = sectionItem as Record<string, unknown>;
      if (sectionRecord.videoRenderer && typeof sectionRecord.videoRenderer === "object") {
        renderers.push(sectionRecord.videoRenderer as Record<string, unknown>);
      }
    }
  }
...
async function fetchYouTubeSearchResults(query: string) {
  ...
  const renderers = collectVideoRenderers(primaryItems);
...
async function fetchYouTubeSearchPayload(query: string): Promise<YouTubeSearchPayload> {
...
async function resolveVideoMetadata(videoId: string) {
